...
var getItem = function (tab, index, className, active, onClick, theme) {
    ...
    return (React.createElement(TabListItem, { key: "item".concat(index), theme: theme, className: "tab__item".concat(className ? ' ' + className : '') },
        React.createElement(TabListButton, { id: "btn".concat(index), className: "tab__button".concat(isActive ? ' tab__button--active' : ''), active: isActive, title: tab, value: index, onClick: onClick, type: 'button', role: 'tab', name: tab, "aria-controls": "tabpane-".concat(index), "aria-selected": isActive, tabIndex: isActive ? 0 : -1, theme: theme }, tab)));
};
